...
contract DEX {
    ...
    mapping(address => uint256) public liquidity; // Liquidity provided by each address
    ...
    function price(uint256 xInput, uint256 xReserves, uint256 yReserves) public pure returns (uint256 yOutput) {
        // Apply 0.3% fee: multiply input by 997 (keeping 99.7%)
    ...
    function ethToToken() public payable returns (uint256 tokenOutput) {
        ...
        uint256 ethReserve = address(this).balance - msg.value; // ETH reserve before this transaction
    ...
    function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
        ...
        require(token.transferFrom(msg.sender, address(this), tokenInput), "tokenToEth: transferFrom failed");
    ...
    function deposit() public payable returns (uint256 tokensDeposited) {
        ...
        uint256 ethReserve = address(this).balance - msg.value; // ETH reserve before this deposit
        uint256 tokenReserve = token.balanceOf(address(this));
        
        // Calculate tokens needed to maintain the ratio
        tokensDeposited = (msg.value * tokenReserve) / ethReserve;
        
        // Calculate liquidity minted proportionally
        ...
        // Transfer tokens from depositor to DEX
        require(token.transferFrom(msg.sender, address(this), tokensDeposited), "deposit: transferFrom failed");
    ...
    function withdraw(uint256 amount) public returns (uint256 ethAmount, uint256 tokenAmount) {
        ...
        // Calculate ETH and tokens to withdraw proportionally
